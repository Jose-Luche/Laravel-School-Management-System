...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
                                        ...
                                        <div class="controls">
                                            <select name="employee_id"  required="" class="form-control">
                                                ...
                                                @foreach ($employees as $employee)
                                                    <option value="{{ $employee->id }}" {{ ($editData->employee_id == $employee->id)
                                        ...
                                        <div class="controls">
                                            <select name="leave_purpose_id" id="leave_purpose_id"  required="" class="form-control">
                                                ...
                                                @foreach ($leave_purpose as $leave)
                                                    <option value="{{ $leave->id }}" {{ ($editData->leave_purpose_id ==
                                                    $leave->id)?'selected':'' }}>{{ $leave->name }}</option>
                                                @endforeach
                                                    <option value="0">New Purpose</option>
                                                
        
                                            </select> <br>
                                            <input type="text" name="name" id="add" class="form-control" placeholder="Type Purpose" style="display:none;">
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <!-- Adding Extra Input fields when an option has been selected -->
